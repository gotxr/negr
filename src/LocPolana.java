...
public class LocPolana extends JPanel implements KeyListener {
    ...
    double[] pollesPortals = {0.865, 0.188, 0.01, 0.05};
    double[] domPortals = {0.15, 0.178, 0.01, 0.05};

    List<Enemy> enemies = new ArrayList<>();

    //конструктор
    public LocPolana(Player existingPlayer) {
    ...
    private void init() {
        ...
        fon = ImageLoader.loadImage("/assets/location_u_doma.png");
    ...
    private void updateGame() {
        if (isPlayerInPortal(new double[][]{pollesPortals})) {
            player.x = 220;
            player.y = 210;
        ...
        if (isPlayerInPortal(new double[][]{domPortals})) {
            player.x = 830;
            player.y = 330;
            switchToLocation(new LocationDom(player));
            return;
        }
    ...
    private void saveGame() {
        GameSave.save(player, 2); // 2 = поляна
    }
